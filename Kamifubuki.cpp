...
void InitKamifubuki() {
	...
	DirectX::CreateWICTextureFromFile(
		Renderer::GetDevice(),
		L"assets/texture/kamifubuki.png",
		nullptr,
		&g_Texture);
...
void DrawKamifubuki() {
	...
	int kamifubuki = count / 10;

	int number = kamifubuki % 12;

	//テクスチャ座標算出
	float tx = 1.0f / 3;
	float ty = number % 12 * (1.0f / 12);
	...
	vertex[0].Normal = Vector3(0.0f, 1.0f, 0.0f);
	vertex[0].Diffuse = Vector4(1.0f, 1.0f, 1.0f, 0.5f);
	vertex[0].TexCoord = Vector2(tx, ty);

	vertex[1].Position = Vector3(x + width, y, 0.0f);
	vertex[1].Normal = Vector3(0.0f, 1.0f, 0.0f);
	vertex[1].Diffuse = Vector4(1.0f, 1.0f, 1.0f, 0.5f);
	vertex[1].TexCoord = Vector2(tx + 1.0f / 3, ty);

	vertex[2].Position = Vector3(x, y + height, 0.0f);
	vertex[2].Normal = Vector3(0.0f, 1.0f, 0.0f);
	vertex[2].Diffuse = Vector4(1.0f, 1.0f, 1.0f, 0.5f);
	vertex[2].TexCoord = Vector2(tx, ty + 1.0f / 12);

	vertex[3].Position = Vector3(x + width, y + height, 0.0f);
	vertex[3].Normal = Vector3(0.0f, 1.0f, 0.0f);
	vertex[3].Diffuse = Vector4(1.0f, 1.0f, 1.0f, 0.5f);
	vertex[3].TexCoord = Vector2(tx + 1.0f / 3, ty + 1.0f / 12);
